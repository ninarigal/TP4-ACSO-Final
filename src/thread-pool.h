 ...
 */
 ...
 private:
  ...
  struct Worker {
    std::condition_variable cv; // condition variable for worker
    std::function<void(void)> task; // task to execute
    bool busy = false; // flag to indicate if worker is busy
    std::mutex mutex; // mutex for worker
  };
